...
@section('content')
    <div class="container">
        <div class="col-md-12">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form class="category" method="POST" action="{{ route('edit_category', $category->id)}}" enctype="multipart/form-data">
                        @csrf
                                                ...
                                                <div class='{{$language === app()->getLocale()? "tab-pane active":"tab-pane"}}' id="{{$language}}" role="tabpanel">
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-10 mb-10 mb-sm-10">
                                                            <input type="text" class="form-control @error('name_'.$language) is-invalid @enderror" name="name_{{$language}}" value="{{ $category->translate($language)->name }}" autocomplete="name_{{$language}}" placeholder={{__('translations.category_name')}} autofocus>
    ...
    <script type="text/javascript">
        ...
        if (image){
            initialPreview = ['/storage/uploads/categories/' + image];
        ...
        $(".kv-file-remove").click(function (e) {
            ...
            e.preventDefault();
            let id = '{{$category->id}}'
            let link = '{{$category->image}}'
            let type = 'image';
            let kv_file = $(this)
            $.ajax({
                type: "post",
                url: "/admin/categories/remove-category-image",
