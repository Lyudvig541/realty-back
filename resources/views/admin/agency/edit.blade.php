...
@section('content')
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h1 class="h4 m-0 font-weight-bold text-primary text-center">
                        ...
                        {{ __('translations.agency') }}
                    </h1>
                </div>
                <div class="card-body">
                    <form class="agency" method="POST" action="{{ route('edit_agency', $agency->id)}}" enctype="multipart/form-data">
                        @csrf
                                                ...
                                                <div class='{{$language === app()->getLocale()? "tab-pane active":"tab-pane"}}' id="{{$language}}" role="tabpanel">
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-10 mb-10 mb-sm-10">
                                                            <input type="text" class="form-control @error('name_'.$language) is-invalid @enderror" name="name_{{$language}}" value="{{$agency->translate($language)->name}}" autocomplete="name_{{$language}}" placeholder="{{__('translations.agency_name')}}" autofocus>
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-sm-10 mb-10 mb-sm-10">
                                                            <textarea type="text" class="form-control @error('description_'.$language) is-invalid @enderror" name="description_{{$language}}" autocomplete="description_"{{$language}} placeholder="{{ __('translations.description') }}" autofocus>{{$agency->translate($language)->description}}</textarea>
                                                        ...
                                                        <div class="col-sm-4 mb-4 mb-sm-4">
                                                            <select class="form-control" name="country_id">
                                                                @foreach($countries as $country)
                                                                    <option value="{{$country->id}}" {{$country->id == $agency->country_id ? 'selected' : ''}}>{{$country->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-sm-4 mb-4 mb-sm-4">
                                                            <select class="form-control" name="state_id" id="select_state">
                                                                ...
                                                                @foreach($states as $state)
                                                                    <option value="{{$state->id}}" {{$state->id == $agency->state_id ? 'selected' : ''}}>{{$state->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="col-sm-4 mb-4 mb-sm-4">
    ...
    <script type="text/javascript">
        ...
        if (image){
            initialPreview = ['/storage/uploads/agencies/' + image];
