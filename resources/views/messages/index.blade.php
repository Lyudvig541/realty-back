...
@section('content')
    <div class="container">
        <div class="panel messages-panel">
                    ...
                    <div id="inbox" class="contacts-outter-wrapper tab-pane active">
                        <div class="contacts-outter">
                            <ul class="list-unstyled contacts">
                                ...
                                @if($i === 0 || $messages[$i]->user->id !== $messages[$i-1]->user->id)
                                        <li data-target="#{{$messages[$i]->id}}" data-toggle="tab" class="{{$i === 0 ? "active":""}}">
                                            <div class="vcentered info-combo">
                                                <h4 class="no-margin-bottom name">{{$messages[$i]->user->first_name}} {{$messages[$i]->user->last_name}}</h4>
                                            </div>
                                        </li>
                                    @endif
                                @endfor
            ...
            <div class="tab-content">
                @for($i = 0; $i < count($messages) ; $i++)
                    @if($i === 0 || $messages[$i]->user->id !== $messages[$i-1]->user->id)
                        <div class="tab-pane message-body {{$i === 0 ? "active":""}}" id="{{$messages[$i]->id}}" >
                            ...
                            <div class="message-chat">
                                <div class="chat-body" id="my-message-{{$messages[$i]->user_id}}" >
                                    @foreach($messages[$i]->user->userMessages()->where('broker_id',$messages[$i]->broker_id)->orderBY('created_at','desc')->get() as $message)
                                        @if(!$message->write_broker)
                                            <div class="message info">
                                                <img alt="" class="img-circle medium-image" src="https://bootdey.com/img/Content/avatar/avatar1.png">
                                                <div class="message-body" style="height: auto">
                                                    <div class="message-info">
                                                        <h4> {{$messages[$i]->user->first_name}} {{$messages[$i]->user->last_name}} </h4>
                                                    </div>
                                                    <hr>
                                ...
                                <div class="chat-footer">
                                    <textarea id="message-{{$messages[$i]->user_id}}" class="send-message-text"></textarea>
                                    <button type="button" class="send-message-button btn-info" onclick="sendMessage({{$messages[$i]->broker_id}}, {{$messages[$i]->user_id}})"> <i class="fa fa-send"></i> </button>
                                </div>
                            </div>
                        </div>
                    @endif
    ...
    <script>
            function sendMessage(broker_id ,user_id){
                $.ajaxSetup({
                    headers: {
                ...
                $.ajax({
                    ...
                    data: {broker_id: broker_id,user_id:user_id,message:document.getElementById('message-'+user_id).value},
                    success: function (data) {
                        if (data.alert === 'success') {
                            ...
                            div_message_text.textContent = document.getElementById('message-'+user_id).value
                            h4.innerText = "{{\Illuminate\Support\Facades\Auth::user()->first_name }}  {{ \Illuminate\Support\Facades\Auth::user()->last_name}}"
                            ...
                            document.getElementById("my-message-" + user_id).appendChild(div_my_message)
                            document.getElementById('message-'+user_id).value = ' '
